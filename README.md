...
Formatted, versioned legislative data for Louisiana. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Louisiana — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/la-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`la-legislation`) — reads the scraper repo's output and:
...
Current settings for Louisiana:

- **State code**: `la`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/la-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:la/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:la/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:la/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:la/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/la-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
